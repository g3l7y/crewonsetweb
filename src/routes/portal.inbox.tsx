...
export const Route = createFileRoute("/portal/inbox")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Review player activity and exchange messages with friends." },
    ],
  }),
  component: InboxPage,
});
...
type MailRow = PlayerMail | (InboxNotification & {
...
function InboxPage() {
  ...
  const currentUsername = profileQuery.data?.username ?? profileQuery.data?.displayName ?? sessionQuery.data?.username ?? (mockMode ? "CAMERA_PRO" : "");
  const currentEmail = profileQuery.data?.email ?? sessionQuery.data?.email ?? (mockMode ? "player@crewonset.com" : "");
  const currentPlayerId = profileQuery.data?.playFabId ?? sessionQuery.data?.playFabId ?? (mockMode ? "MOCK-PLAYER-001" : "");
  ...
  const visibleNotifications = useMemo(
    () =>
      sourceNotifications.filter((notification) =>
  ...
  const unreadNotifications = activityNotifications.filter((notification) => !notification.read).length;
  ...
  function markNotificationRead(id: string) {
    if (mockMode) {
      setDemoNotifications((current) => current.map((item) => (item.id === id ? { ...item, read: true } : item)));
    } else {
  ...
  function markAllNotificationsRead() {
    if (mockMode) {
      ...
      void fetch("/api/notifications", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ids: realNotifications.filter(isActivityNotification).map((item) => item.id) }),
      });
    }
  }
  ...
  function markMailRead(mail: MailRow) {
    ...
    if (visibleNotifications.some((notification) => notification.id === mail.id)) {
      ...
      setDemoMail((current) => current.map((item) => (item.id === mail.id ? { ...item, read: true } : item)));
    }
  }
    ...
    <div className="portal-page portal-title-page min-h-screen bg-[#0b1426] px-4 py-8 text-white sm:px-6 sm:py-10 lg:px-8">
      <div className="portal-title-container mx-auto max-w-[1500px]">
        <header className="portal-title-header">
          <p className="portal-title-eyebrow text-xs font-black tracking-[.18em] text-coral">COMMUNICATION</p>
          <h1 className="portal-title-heading mt-2 text-4xl font-black uppercase tracking-tight text-white sm:text-5xl">Inbox</h1>
            ...
            <div className="flex shrink-0 flex-wrap items-center justify-between gap-4 border-b border-white/10 px-5 py-4 sm:px-6">
              <div className="flex flex-wrap gap-2" role="tablist" aria-label="Inbox sections">
                <button
                  ...
                  className={activeTab === "notifications" ? "inline-flex items-center gap-2 rounded-md border-2 border-[#f0573d] bg-[#f0573d] px-4 py-2 text-xs font-black uppercase tracking-wide text-[#0a0e19]" : "inline-flex items-center gap-2 rounded-md border border-white/15 px-4 py-2 text-xs font-black uppercase tracking-wide text-white/70 transition hover:bg-white/10 hover:text-white"}
                >
                  <Bell className="size-4" /> Notifications
                  {unreadNotifications > 0 && <span className="rounded-full bg-coral px-1.5 py-0.5 text-[10px] text-white">{unreadNotifications}</span>}
                </button>
                <button
                  type="button"
                  role="tab"
                  ...
                  className={activeTab === "mail" ? "inline-flex items-center gap-2 rounded-md border-2 border-[#fbbf24] bg-[#fbbf24] px-4 py-2 text-xs font-black uppercase tracking-wide text-[#0a0e19]" : "inline-flex items-center gap-2 rounded-md border border-white/15 px-4 py-2 text-xs font-black uppercase tracking-wide text-white/70 transition hover:bg-white/10 hover:text-white"}
                >
                  <Mail className="size-4" /> Mail
                  {unreadMail > 0 && <span className="rounded-full bg-coral px-1.5 py-0.5 text-[10px] text-white">{unreadMail}</span>}
                </button>
              </div>
                  ...
                  activityNotifications.map((notification) => {
                    ...
                    return (
                      <button
                        ...
                        className={notification.read ? "flex w-full items-start gap-4 px-5 py-5 text-left opacity-70 transition hover:bg-white/[.04] sm:px-6" : "flex w-full items-start gap-4 px-5 py-5 text-left transition hover:bg-white/[.04] sm:px-6"}
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="flex items-start justify-between gap-4">
                            <span className="text-sm font-black text-white sm:text-base">{notification.title}</span>
                            {!notification.read && <span className="mt-1 size-2 shrink-0 rounded-full bg-coral" />}
                          </span>
                          <span className="mt-1 block text-sm leading-relaxed text-white/60">{notification.body}</span>
              ...
              <div className="grid min-h-0 flex-1 overflow-hidden lg:grid-cols-[280px_minmax(0,1fr)]">
                <aside className="flex min-h-0 flex-col overflow-hidden border-b border-white/10 lg:border-b-0 lg:border-r">
                  <div className="shrink-0 border-b border-white/10 px-5 py-5 sm:px-6">
                    ...
                    <p className="mt-1 text-xs text-white/45">Select one to open the conversation</p>
                  </div>
                  <div className="min-h-0 flex-1 overflow-y-scroll divide-y divide-white/10">
                    {allFriends.map((friend) => {
                      ...
                      return (
                        <button
                          ...
                          className={selected ? "flex w-full items-center gap-3 bg-white/[.08] px-5 py-4 text-left transition hover:bg-white/[.1]" : "flex w-full items-center gap-3 px-5 py-4 text-left transition hover:bg-white/[.05]"}
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="flex items-center gap-2">
                              <span className="truncate text-sm font-black text-white">{friend.name}</span>
                              <span className={isOnline ? "size-2 shrink-0 rounded-full bg-[#4bc4b4]" : "size-2 shrink-0 rounded-full bg-white/25"} />
                            </span>
                            <span className="mt-1 block truncate text-[10px] font-bold uppercase tracking-wide text-white/40">
                              {isOnline ? "Online" : "Offline"} · Level {friend.level} · {friend.role}
                            </span>
                          </span>
                          {mailRows.some((mail) => mail.kind === "friend" && !mail.read && (mail.senderUsername === friend.name || mail.recipientUsername === friend.name)) && (
                    ...
                    {mailRows.some((mail) => mail.kind === "admin") && (
                      <button
                        ...
                        className={selectedAdminMail ? "flex w-full items-center gap-3 bg-white/[.08] px-5 py-4 text-left transition hover:bg-white/[.1]" : "flex w-full items-center gap-3 px-5 py-4 text-left transition hover:bg-white/[.05]"}
                      >
                ...
                <section className="flex min-h-0 min-w-0 flex-col">
                  {!recipient ? (
                    <div className="flex flex-1 flex-col items-center justify-center px-6 py-16 text-center">
                      ...
                      <p className="mt-5 text-sm font-black uppercase tracking-wide text-white/45">Select a friend to start chatting</p>
                    </div>
                  ) : (
                    <>
                      <header className="flex items-center gap-3 border-b border-white/10 px-5 py-4 sm:px-6">
                        {selectedFriend ? (
                          <>
                            <span className="relative size-10 shrink-0 overflow-hidden rounded-full border border-white/15 bg-yellow">
                              <Image
                                src={selectedFriend.profileImage || getProfileArtwork(selectedFriend.name)}
                            ...
                            <span>
                              <span className="flex items-center gap-2 text-sm font-black uppercase text-white">
                                {selectedFriend.name}
                                <span className={selectedFriend.online && selectedFriend.showStatus !== false ? "size-2 rounded-full bg-[#4bc4b4]" : "size-2 rounded-full bg-white/25"} />
                              </span>
                              <span className="mt-1 block text-[10px] font-bold uppercase tracking-wide text-white/40">Level {selectedFriend.level} · {selectedFriend.role}</span>
                            </span>
                          </>
                        ) : (
                          <>
                            <span className="grid size-10 place-items-center rounded-full bg-coral/15 text-coral"><MailOpen className="size-5" /></span>
                            <span>
                              <span className="block text-sm font-black uppercase text-white">Administrator</span>
                              <span className="mt-1 block text-[10px] font-bold uppercase tracking-wide text-white/40">Inbox messages</span>
                            </span>
                          </>
                        )}
                      ...
                      <div className="min-h-0 flex-1 overflow-y-scroll px-5 py-6 sm:px-8">
                        {selectedMessages.length === 0 ? (
                          <div className="flex h-full min-h-[18rem] flex-col items-center justify-center text-center">
                            ...
                            {selectedFriend && <p className="mt-2 text-sm text-white/40">Send a message below to begin.</p>}
                          </div>
                        ) : (
                          <div className="mx-auto grid max-w-2xl gap-4">
                            {[...selectedMessages].reverse().map((mail) => {
                              const fromCurrentPlayer = mail.senderUsername.toLowerCase() === currentUsername.toLowerCase();
                              return (
                                <div key={mail.id} className={fromCurrentPlayer ? "ml-auto flex max-w-[88%] items-start gap-2" : "mr-auto flex max-w-[88%] items-start gap-2"}>
                                  <span className={fromCurrentPlayer ? "relative order-2 mt-1 size-8 shrink-0 overflow-hidden rounded-full border border-white/15 bg-yellow" : "relative mt-1 size-8 shrink-0 overflow-hidden rounded-full border border-white/15 bg-yellow"}>
                                    <Image
                                  ...
                                  <div className="min-w-0">
                                    <div className={fromCurrentPlayer ? "rounded-2xl rounded-br-sm bg-coral px-4 py-3 text-white" : "rounded-2xl rounded-bl-sm bg-white/[.08] px-4 py-3 text-white"}>
                                      <p className="text-sm leading-relaxed">{mail.body}</p>
                                    </div>
                                    <p className={fromCurrentPlayer ? "mt-1 text-right text-[10px] font-bold uppercase tracking-wide text-white/35" : "mt-1 text-[10px] font-bold uppercase tracking-wide text-white/35"}>
                      ...
                      {selectedFriend ? (
                        <form onSubmit={sendFriendMessage} className="border-t border-white/10 p-4 sm:p-5">
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              aria-label="Message"
                            />
                            <button type="submit" aria-label="Send message" className="mail-send-button grid size-11 shrink-0 place-items-center rounded-full bg-coral text-white transition hover:bg-yellow hover:text-[#0a0e19]">
